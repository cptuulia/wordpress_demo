...
<script>
    jQuery(document).ready(function() {
        ...
        jQuery('.delete').on("click", function (event) {
            ...
            if (confirm(message)) {
                let url = '/wp-admin/admin.php?page=program%2Feditcategory&category=<?php echo $_GET["category"] ?>&delete=' + parts[0];
    ...
    function moveRow(row, direction, type) {
        ...
        numberOfItems = jQuery(".kamergro_programIndex tr").length

        if (OrderingOfRowToReplace < 1 || OrderingOfRowToReplace >= numberOfItems) {
            return;
        }
        ...
        selector = selector = '.itemOrdering';
